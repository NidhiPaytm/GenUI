...
import { graph } from "@opencanvas/agents/src/open-canvas";
...
ls.describe("codegen", () => {
  ls.test(
    ...
    async ({ inputs }) => {
      const rewriteArtifactNode = graph.nodes.rewriteArtifact;
      const res = await rewriteArtifactNode.invoke(inputs, {
        configurable: {
          customModelName: "gpt-4o-mini",
        },
      });
