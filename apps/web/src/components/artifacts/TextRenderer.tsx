...
//       ; (divRef.current as any).innerHTML = '' // Clear the container
//       ; (divRef.current as any).appendChild(slotHtml) // Append the new content
//   }, [html, divRef])

//   return React.createElement('div', { ...rest, ref: divRef })
// }

export const ExtCode = createReactBlockSpec(
  {
    type: "extcode",
    propSchema: {
      contentCode: {
  ...
  },
  {
    render: (props) => {
      return (
        <div className="w-full h-[80vh] overflow-hidden">
          {/* <DangerouslySetHtmlContent html={props.block.props.contentCode} /> */}
          <iframe srcDoc={props.block.props.contentCode} className="w-full h-full"
            sandbox="allow-scripts allow-same-origin allow-popups allow-forms allow-modals"
            key={props.block.props.contentCode}
          />
        </div>
      );
    },
    parse(el) {
      ...
      // <ExtCode><Content /></ExtCode>
      if (el.tagName === 'PRE') {
        const code = el.childNodes[0] as HTMLElement | undefined
        if (code?.nodeType === 1 && code.tagName === 'CODE') {
          // const [, contentCode] = /<Content>([\S\s\n]*)<\/Content>/m.exec(code.innerText) || []
          const contentCode = code.innerText
          console.log(code.innerText, contentCode)
          if (contentCode) {
            return {
              contentCode
            }
    ...
    toExternalHTML(props) {
      return <pre><code data-language="ExtCode">
        {
          `<Content>${props.block.props.contentCode}</Content>`
        }
      </code></pre>
    }
  }
