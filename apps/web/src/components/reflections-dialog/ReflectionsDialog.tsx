...
import { BrainCog, Loader, Plus } from "lucide-react";
...
interface AddReflectionFormProps {
  onSubmit: (styleRules: string[], content: string[]) => Promise<void>;
  onCancel: () => void;
}

function AddReflectionForm({ onSubmit, onCancel }: AddReflectionFormProps) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Enter style rules (one per line)"
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Enter content reflections (one per line)"
          className="min-h-[100px]"
        />
      </div>
      <div className="flex justify-end gap-2">
        ...
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? "Adding..." : "Add Reflections"}
        </Button>
      </div>
    </form>
  );
...
export function ReflectionsDialog(props: ReflectionsDialogProps) {
  ...
  const handleAddReflections = async (styleRules: string[], content: string[]) => {
    if (!selectedAssistant) {
      toast({
        title: "Error",
        description: "Assistant ID not found.",
    ...
    try {
      await addReflections(selectedAssistant.assistant_id, styleRules, content);
      toast({
        title: "Success",
        description: "Reflections added successfully.",
        duration: 3000,
      });
      setIsAdding(false);
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to add reflections.",
        variant: "destructive",
        duration: 5000,
      });
    }
      ...
      <DialogContent className="max-w-xl p-8 bg-white rounded-lg shadow-xl">
                ...
                style={{
                  ...(iconData
                    ? {
                      ...
                      backgroundColor: `${iconData.iconColor}20`,
                    }
                    : {
                      color: "#000000",
        ...
        <div className="mt-6 flex justify-between">
          <div className="flex gap-2">
            {!isAdding && (reflections?.content || reflections?.styleRules) && (
            ...
            {!isAdding && (
              ...
              >
                ...
                <TighterText>Add Reflections</TighterText>
              </Button>
            )}
          </div>
          <Button
