...
export default function ModelSelector({
  ...
  const bedrockModelGroup = allAllowedModels.filter(
    (m) => m.config.provider === "bedrock"
  );
      ...
      <PopoverContent className="min-w-[180px] w-[280px] p-0 shadow-md rounded-md">
        <Command>
          <CommandList>
            ...
            {bedrockModelGroup.length > 0 && (
              <CommandGroup heading="AWS Bedrock" className="w-full">
                {bedrockModelGroup.map((model) => {
                  const config = modelConfigs[model.name];
                  return (
                    <CommandModelItem
                      key={model.name}
