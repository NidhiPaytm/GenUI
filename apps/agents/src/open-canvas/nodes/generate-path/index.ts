...
import { dynamicDeterminePath } from "./dynamic-determine-path";
import {
  convertContextDocumentToHumanMessage,
  fixMisFormattedContextDocMessage,
} from "./documents";
import { getStringFromContent } from "@/utils";
import { includeURLContents } from "./include-url-contents";
...
export async function generatePath(
  ...
  const routingResult = await dynamicDeterminePath({
    state: {
      ...state,
      _messages: newInternalMessageList as BaseMessage[],
    },
    newMessages,
    config,
  });
  ...
  const messages = newMessages.length
    ? {
      ...
      _messages: [...newInternalMessageList, ...newMessages] as BaseMessage[],
    }
    : {
      _messages: newInternalMessageList as BaseMessage[],
    };
